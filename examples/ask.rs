...
//!
//! ```text
//! cargo run --example ask <ip>:<port> [<address> [<from>]]
//! ```
//!
//! `<ip>:<port>` is where the host's CHUDP socket is, by name or by IP
//! address. `<address>` is the host's Chaos address, in octal, and 3060
//! unless given. Each request is a simple transaction (`PROTOCOLS.md`): an
//! RFC for the contact name, and the ANS that comes back, or nothing within
//! three seconds.
//!
//! **It sends from a Chaos address of its own**, `<from>` in octal, or else
//! the asked host's subnet with host 376. A hub learns that address at this
...
/// The host asked when no address is given: System 100's file host,
/// `MIT-OZ`, as the README configures it.
const DEFAULT_HOST: u16 = 0o3060;
...
fn main() {
    ...
    let Some(at) = args[0].to_socket_addrs().ok().and_then(|mut a| a.next()) else {
        eprintln!("ask: {}: not an endpoint, which is <ip>:<port>", args[0]);
        exit(2);
    };
    let host = args.get(1).map_or(DEFAULT_HOST, |a| octal(a));
    let from = args.get(2).map_or((host & 0o177400) | 0o376, |a| octal(a));
...
/// `word` as a Chaos address in octal, or the usage and exit code 2.
fn octal(word: &str) -> u16 {
    u16::from_str_radix(word, 8).unwrap_or_else(|_| {
        eprintln!("ask: {word}: not a Chaos address, which is octal");
        exit(2)
    })
}

fn usage() -> ! {
    eprintln!("usage: cargo run --example ask <ip>:<port> [<address> [<from>]]");
    eprintln!("       the addresses in octal; <address> is 3060 unless given");
    exit(2);
}
